...
local em = {
  ...
  loop_when_navigating = true,          -- Loop when navigating through list
  ...
  text_color = {
    -- Gruvbox text colors (ASS uses BGR format internally)
    default = 'b2dbeb', -- Gruvbox Foreground (ebdbb2)
    accent = '2fbdfa',  -- Gruvbox Yellow (fabd2f)
    current = '26bbb8', -- Gruvbox Green (b8bb26)
    comment = '748392', -- Gruvbox Gray (928374)
  },
...
function em:filter_wrapper()
  ...
  if self.selection_change then
    self:selection_change(self:current()[1])
  end
end
...
function em:change_selected_index(num)
  ...
  self:set_from_to()
  if self.selection_change then
    self:selection_change(self:current()[self.list.pointer_i])
  end
  self:update()
end
...
function em:update(err_code)
  ...
  local menu_height = line_height * (self.lines_to_show + 1) + self.menu_y_padding * 2
  local menu_y_pos = (wh - menu_height) / 2
  local menu_width = ww * 0.6
  local menu_x_pos = (ww - menu_width) / 2
  ...
  local function get_background()
    ...
    -- Gruvbox dark bg color (282828) with transparent opacity (B3)
    a:append('{\\1c&H282828\\1a&HB3}') 
    a:pos(0, 0)
    a:draw_start()
    a:rect_cw(menu_x_pos, menu_y_pos, menu_x_pos + menu_width, menu_y_pos + menu_height)
    a:draw_stop()
    return a.text
  end
  ...
  local function get_search_header()
    ...
    a:append('{\\clip(' .. menu_x_pos .. ',' .. menu_y_pos .. ',' .. (menu_x_pos + menu_width) .. ',' .. (menu_y_pos + menu_height) .. ')}')
    a:pos(menu_x_pos + self.menu_x_padding, menu_y_pos + self.menu_y_padding)
  ...
  local function get_list()
    ...
    local function apply_highlighting(y)
      ...
      a:append('{\\1c&H545c66\\1a&H80}') -- Gruvbox Highlight (665c54) & opacity
      a:pos(0, 0)
      a:draw_start()
      a:rect_cw(menu_x_pos, y, menu_x_pos + menu_width, y + self.font_size)
      a:draw_stop()
    end
    ...
    for i = self.list.show_from_to[1], self.list.show_from_to[2] do
      ...
      a:append(self:reset_styles())
      a:append('{\\clip(' .. menu_x_pos .. ',' .. menu_y_pos .. ',' .. (menu_x_pos + menu_width) .. ',' .. (menu_y_pos + menu_height) .. ')}')
      a:pos(menu_x_pos + self.menu_x_padding, y_offset)
...
function em:get_line(i, v) -- [i]ndex, [v]alue
  ...
  if i == self.list.pointer_i then
    style = 'accent'
  elseif self.list.current_i == v[self.index_field] then
    style = 'current'
  else
    style = 'default'
  end
