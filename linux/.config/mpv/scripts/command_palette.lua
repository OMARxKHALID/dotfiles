...
mp.register_script_message("show-command-palette", function (name)
    ...
    elseif name == "Recent Files" then
        local log_path = mp.command_native({"expand-path", "~~/history.log"})
        local f = io.open(log_path, "r")
        if not f then
            mp.osd_message("No history found", 2)
            return
        end
        ...
        for i = #all_lines, 1, -1 do
            ...
            -- Log format: "DD.MM.YYYY HH:MM XXX path" (21 char prefix + path)
            if #line > 22 then
                local p = line:sub(22):gsub("[\r\n]", "")
                p = p:gsub("^%s+", ""):gsub("%s+$", "")
            ...
            if #paths >= 50 then break end
        end

        if #paths == 0 then
            mp.osd_message("No recent files", 2)
            return
        end
        ...
        menu_content.list = list
        function menu:selection_change(tbl) end
        function menu:submit(tbl)
            mp.commandv("loadfile", tbl.path)
        end
    elseif name == "Fonts" then
        local fonts = {
        ...
        local current_font = mp.get_property("sub-font")
        for i, font in ipairs(fonts) do
            table.insert(menu_content.list, { index = i, content = font })
            if font == current_font then
                menu_content.current_i = i
            end
        end

        function menu:selection_change(tbl)
            if tbl then
                mp.set_property("sub-font", tbl.content)
                mp.set_property("osd-font", tbl.content)
                -- Apply Netflix Style Color/Border/Shadow to all
                mp.set_property("sub-color", "#FFFFFFFF")
                mp.set_property("sub-border-color", "#FF262626")
                mp.set_property("sub-border-size", "2")
                mp.set_property("sub-shadow-offset", "1")
                mp.set_property("sub-shadow-color", "#33000000")
                mp.set_property("sub-spacing", "0.5")
            end
        end
        ...
        function menu:submit(tbl)
            ...
            -- Persist to mpv.conf (Linux specific)
            local config_path = mp.command_native({"expand-path", "~~/mpv.conf"})
            mp.commandv("run", "sed", "-i", "s/^sub-font=.*/sub-font=\"" .. tbl.content .. "\" /", config_path)
            mp.commandv("run", "sed", "-i", "s/^osd-font=.*/osd-font=\"" .. tbl.content .. "\" /", config_path)

            mp.commandv("show-text", "Font PERSISTED to config: " .. tbl.content)
        end
    ...
    elseif name == "Toggle Auto-Subtitle" then
        mp.command("script-binding autosub/toggle_autosub")
